using AI.DocumentIntelligence.Application.Abstractions.Export;
using AI.DocumentIntelligence.Application.Common.Messaging;
using AI.DocumentIntelligence.Application.Contracts.Export;
using AI.DocumentIntelligence.Domain.Common;

namespace AI.DocumentIntelligence.Application.Features.Export.ExportAnalysis;

/// <summary>
/// Handles <see cref="ExportAnalysisCommand"/> by delegating to <see cref="IExportService"/>.
/// The handler is intentionally thin — all formatting logic lives in the Infrastructure layer.
/// </summary>
internal sealed class ExportAnalysisCommandHandler(IExportService exportService)
    : ICommandHandler<ExportAnalysisCommand, ExportDocumentResult>
{
    /// <inheritdoc />
    public Task<Result<ExportDocumentResult>> Handle(
        ExportAnalysisCommand request,
        CancellationToken cancellationToken) =>
        exportService.ExportAnalysisAsync(
            request.Result,
            request.Format,
            request.Title,
            cancellationToken);
}
